...
Future<void> exportPlaylist(
  ...
  try {
    final String dirPath = await Picker.selectFolder(
      context: context,
      message: CustomLocalizations.of(context).selectExportLocation,
    );
    if (dirPath == '') {
      ShowSnackBar().showSnackBar(
        context,
        '${CustomLocalizations.of(context).failedExport} "$showName"',
      );
      return;
    }
    ...
    ShowSnackBar().showSnackBar(
      context,
      '${CustomLocalizations.of(context).exported} "$showName"',
    );
  } catch (e) {
    Logger.root.severe('Error while exporting playlist', e);
    ShowSnackBar().showSnackBar(
      context,
      CustomLocalizations.of(context).failedExport,
    );
  }
}
...
Future<void> sharePlaylist(
  ...
  await Share.shareXFiles(
    files,
    text: CustomLocalizations.of(context).playlistShareText,
  );
...
Future<List> importFilePlaylist(
  ...
  try {
    ...
    if ((pickFile || path == null) && context != null) {
      try {
        temp = await Picker.selectFile(
          context: context,
          // ext: ['json'],
          message: CustomLocalizations.of(context).selectJsonImport,
        );
      } catch (e) {
        temp = await Picker.selectFile(
          context: context,
    ...
    if (context != null) {
      ShowSnackBar().showSnackBar(
        context,
        '${CustomLocalizations.of(context).importSuccess} "$playlistName"',
      );
    }
    return playlistNames;
  } catch (e) {
